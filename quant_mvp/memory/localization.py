...
_EXACT_TEXT = {
    ...
    "The default-project data blocker should be cleared before expanding into multiple subagents.": "默认项目的数据 blocker 还没清掉，现在扩展多个 subagent 只会增加协作成本。",
    "The current default-project blocker does not justify extra coordination yet.": "当前默认项目的 blocker 还不值得为它再加一层协作开销。",
    "Stay effectively OFF until validated bars restore independent work packages.": "在可用 validated bars 恢复前，subagent 保持有效 OFF，先不要拆分。",
...
def humanize_text(value: object) -> str:
    ...
    replacements = {
        ...
        "Own provider, ingestion, cleaning, and data coverage diagnostics without changing strategy logic.": "负责数据提供商、拉取、清洗与覆盖诊断，但不改动策略逻辑。",
        "Check strategy entrypoints, defaults, and documentation for drift.": "检查策略入口、默认配置和文档是否与现状研究发生漂移。",
        "Own leakage, robustness, baseline, and promotion-gate verification work.": "负责泄漏、稳健性、baseline 和 promotion gate 的验证工作。",
        "Keep tracked memory, handoff, and migration prompts concise and accurate.": "保持 tracked memory、handoff 和 migration prompt 简洁、准确。",
        "Investigate missing tools, policy files, and reproducibility boundaries before anything is added.": "在增加任何东西前，先查缺失工具、policy 文件和可复现边界。",
        "Merge compatible workstreams, reduce overlap, and close out temporary subagents.": "合并可兼容的工作流、减少重复，并关停已完成的临时 subagent。",
