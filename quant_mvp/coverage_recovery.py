    ...
    *,
    ...
    strategy_board = "\n".join(
        [
            ...
            "",
            "## ??",
            f"- `baseline_limit_up` | ???? | ?? `{stage_decision.stage}` | ??=hold",
            f"- ?? blocker: {baseline_status}",
            "",
            "## ??",
            f"- `risk_constrained_limit_up` | ???? | ???????????",
            f"- `tighter_entry_limit_up` | ???? | ???????????",
            "",
            "## ?????",
            f"- ?? coverage: `{post_summary['covered_symbols']} / {post_summary['universe_symbols']} = {_fmt_ratio(post_summary['coverage_ratio'])}`",
            f"- ?? blocker: {baseline_status}",
            "- ??: ?????????????????????? universe-reset ???",
        ],
    )
