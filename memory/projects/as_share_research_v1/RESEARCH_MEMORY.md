...
## 已被削弱或否定的策略假设
- `r1_predictive_error_overlay_v2`: F1+R1.2 shared-shell Top6: annualized_return=39.08%, max_drawdown=25.47%, sharpe=1.5151; decision=reject_r1_v2_and_promote_f2_next.
...
- ??? F2.1 verifier ??????????????????????
- ??? Excel ????????????????????
- Do not generate .cmd launchers or VBA action modules from the Excel console again.
...
- Do not restore script-based Excel actions.
