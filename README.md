...
- [Backtest Results (5-Year)](#backtest-results-5-year)
...
The strategy can be backtested over a **5-year window (2020-01-01 to 2025-01-01)** for a realistic, out-of-sample style evaluation. Use the dedicated config and run:

```bash
python scripts/run_limit_up_screening.py --project 2026Q1_limit_up --config configs/projects/2026Q1_limit_up_5y.json --no-show --save auto
python scripts/steps/40_make_report.py --project 2026Q1_limit_up
```

Outputs include **summary metrics** (total return, annualized return, Sharpe ratio, max drawdown, Calmar ratio, Sortino ratio, win rate, max drawdown duration) and the plots below.
...
*(Charts are generated by the pipeline; if missing, run the backtest steps above.)*
...
future results. The results and documentation are intended for academic and
